...
$id = $_GET["id"];  // On récupère l'identifiant du youtubeur sélectionné par l'utilisateur
...
// On appelle la fonction "Requete" pour récupérer les données voulues dans les tables "youtubeur" et "video" et en les incluant dans des tableaux distincts

$donnees_youtubeur = Requete("SELECT photo, pseudo, prenom, nom, sexe, pays, chaine, biographie, nb_abonnes, date_creation FROM `youtubeur` WHERE id_youtubeur = ".$id.";", "fetch");
$donnees_videos = Requete("SELECT miniature, titre, photo, pseudo, nb_vues, nb_likes, nb_commentaires, id_video, youtubeur.id_youtubeur FROM `video`, `youtubeur` WHERE youtubeur.id_youtubeur = ".$id." AND youtubeur.id_youtubeur = video.id_youtubeur ORDER BY nb_vues DESC;", "fetchAll");
?>
...
<html lang="fr">
    <head>
        ...
        <title>YouStats - Chaîne de <?php echo($donnees_youtubeur["pseudo"]); // On affiche le pseudo du youtubeur en tant que titre de la page ?></title>
    </head>
    <body>
        <header>
        ...
        <main id="centre">
            ...
            <section id="donnees">
                <div id="textuel">
                    <h1><?php echo($donnees_youtubeur["pseudo"]); ?></h1>
                    <h2><?php if ($donnees_youtubeur["biographie"] == "") { echo("(Aucune biographie)"); } else { echo($donnees_youtubeur["biographie"]); } ?></h2>
                    <div class="details">
                        <ul>    <!-- Nombre d'abonnés -->
                            <li>
                                <svg width="16" height="14" viewBox="0 0 16 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M4 0C6.21 0 8 1.755 8 3.92C8 1.755 9.79 0 12 0C14.21 0 16 1.755 16 3.92C16 7.183 12.766 8.334 8.392 13.528C8.34384 13.585 8.28382 13.6309 8.21611 13.6623C8.14841 13.6938 8.07465 13.7101 8 13.7101C7.92535 13.7101 7.85159 13.6938 7.78389 13.6623C7.71618 13.6309 7.65616 13.585 7.608 13.528C3.234 8.334 0 7.183 0 3.92C0 1.755 1.79 0 4 0Z" fill="white"/>
                                </svg>
                                <p><?php echo(number_format($donnees_youtubeur["nb_abonnes"], 0, ".", " ")); ?></p>
                            </li>
                        </ul>
                        <ul>    <!-- Date de création de la chaîne -->
                            <li>
                                ...
                                <p><?php echo(date("d/m/Y", strtotime($donnees_youtubeur["date_creation"]))); ?></p>
                            </li>
                        </ul>
                    </div>
                    <div class="details">
                        <ul>    <!-- Prénom et nom -->
                            <li>
                                <svg width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path fill-rule="evenodd" clip-rule="evenodd" d="M1.25 15C1.25 15 0 15 0 13.75C0 12.5 1.25 8.75 7.5 8.75C13.75 8.75 15 12.5 15 13.75C15 15 13.75 15 13.75 15H1.25Z" fill="white"/>
                                    <path fill-rule="evenodd" clip-rule="evenodd" d="M7.5 7.5C9.57107 7.5 11.25 5.82107 11.25 3.75C11.25 1.67893 9.57107 0 7.5 0C5.42893 0 3.75 1.67893 3.75 3.75C3.75 5.82107 5.42893 7.5 7.5 7.5Z" fill="white"/>
                                </svg>
                                <p><?php echo($donnees_youtubeur["prenom"]." ".$donnees_youtubeur["nom"]); ?></p>
                            </li>
                        </ul>
                        <ul>    <!-- Sexe -->
                            <li>
                                <?php
                                if ($donnees_youtubeur["sexe"] == "Homme") { ?>
                                <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path fill-rule="evenodd" clip-rule="evenodd" d="M9.5 2C9.22386 2 9 1.77614 9 1.5C9 1.22386 9.22386 1 9.5 1H14.5C14.7761 1 15 1.22386 15 1.5V6.5C15 6.77614 14.7761 7 14.5 7C14.2239 7 14 6.77614 14 6.5V2.70711L9.87145 6.83565C10.5765 7.69734 11 8.79957 11 10C11 12.7614 8.76142 15 6 15C3.23858 15 1 12.7614 1 10C1 7.23858 3.23858 5 6 5C7.20043 5 8.30266 5.42354 9.16435 6.12855L13.2929 2H9.5ZM6 6C3.79086 6 2 7.79086 2 10C2 12.2091 3.79086 14 6 14C8.20914 14 10 12.2091 10 10C10 8.89529 9.55283 7.89598 8.82843 7.17157C8.10402 6.44717 7.10471 6 6 6Z" fill="white" stroke="white" stroke-linecap="round" stroke-linejoin="round"/>
                                </svg>
                                <?php } else { ?>
                                <svg width="16" height="18" viewBox="0 0 16 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path fill-rule="evenodd" clip-rule="evenodd" d="M8 2C5.79086 2 4 3.79086 4 6C4 8.20914 5.79086 10 8 10C10.2091 10 12 8.20914 12 6C12 3.79086 10.2091 2 8 2ZM3 6C3 3.23858 5.23858 1 8 1C10.7614 1 13 3.23858 13 6C13 8.59267 11.0267 10.7245 8.5 10.9753V13H10.5C10.7761 13 11 13.2239 11 13.5C11 13.7761 10.7761 14 10.5 14H8.5V16.5C8.5 16.7761 8.27614 17 8 17C7.72386 17 7.5 16.7761 7.5 16.5V14H5.5C5.22386 14 5 13.7761 5 13.5C5 13.2239 5.22386 13 5.5 13H7.5V10.9753C4.97334 10.7245 3 8.59267 3 6Z" fill="white" stroke="white" stroke-linecap="round"/>
                                </svg>
                                <?php } ?>
                                <p><?php echo($donnees_youtubeur["sexe"]); ?></p>
                            </li>
                        </ul>
                    </div>
                    <div class="details">
                        <ul>    <!-- Pays -->
                            <li>
                                <svg width="13" height="18" viewBox="0 0 13 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M6.5 17.3333C6.5 17.3333 13 11.1735 13 6.5C13 4.77609 12.3152 3.12279 11.0962 1.90381C9.87721 0.684819 8.22391 0 6.5 0C4.77609 0 3.12279 0.684819 1.90381 1.90381C0.684819 3.12279 2.56882e-08 4.77609 0 6.5C0 11.1735 6.5 17.3333 6.5 17.3333ZM6.5 9.75C5.63805 9.75 4.8114 9.40759 4.2019 8.7981C3.59241 8.1886 3.25 7.36195 3.25 6.5C3.25 5.63805 3.59241 4.8114 4.2019 4.2019C4.8114 3.59241 5.63805 3.25 6.5 3.25C7.36195 3.25 8.1886 3.59241 8.7981 4.2019C9.40759 4.8114 9.75 5.63805 9.75 6.5C9.75 7.36195 9.40759 8.1886 8.7981 8.7981C8.1886 9.40759 7.36195 9.75 6.5 9.75Z" fill="white"/>
                                </svg>
                                <p><?php echo($donnees_youtubeur["pays"]); ?></p>
                            </li>
                        </ul>
                        <ul>    <!-- Lien de la chaîne YouTube -->
                            <li>
                                <svg width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <a href="<?php echo($donnees_youtubeur["chaine"]); ?>" target="_blank"><?php echo(substr($donnees_youtubeur["chaine"], 12)); ?></a>
                            </li>
                        </ul>
                    </div>
                </div>
