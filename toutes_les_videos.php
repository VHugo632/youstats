...
if (isset($_GET["tri_type"]) == false || isset($_GET["tri_ordre"]) == false) {  // On affiche les vidéos dans l'ordre par défaut (seulement si l'utilisateur n'a pas effectué de tri)
    $donnees_videos = Requete("SELECT miniature, titre, photo, pseudo, nb_vues, nb_likes, nb_commentaires, id_video, youtubeur.id_youtubeur FROM `video`, `youtubeur` WHERE youtubeur.id_youtubeur = video.id_youtubeur ORDER BY nb_vues DESC;", "fetchAll");
}
else {  // On affiche les vidéos dans l'ordre souhaité (seulement si l'utilisateur a effectué un tri) en fabricant la requête en fonction des choix de l'utiisateur
    $requete_triee = "SELECT miniature, titre, photo, pseudo, nb_vues, nb_likes, nb_commentaires, id_video, youtubeur.id_youtubeur FROM `video`, `youtubeur` WHERE youtubeur.id_youtubeur = video.id_youtubeur";
    switch($_GET["tri_type"]) {
        case "vues" :
            $requete_triee = $requete_triee." ORDER BY nb_vues";
            break;
        case "likes" :
            $requete_triee = $requete_triee." ORDER BY nb_likes";
            break;
        case "commentaires" :
            $requete_triee = $requete_triee." ORDER BY nb_commentaires";
            break;
        default :
            $requete_triee = $requete_triee." ORDER BY duree";
    }
    if ($_GET["tri_ordre"] == "decroissant") {
        $requete_triee = $requete_triee." DESC;";
    }
    else {
        $requete_triee = $requete_triee." ASC;";
    }
    $donnees_videos = Requete($requete_triee, "fetchAll");
}

$compteur_videos = Requete("SELECT COUNT(*) AS `compteur_videos` FROM `video`;", "fetch");  // On récupère le nombre total de vidéos enregistrées dans la base de données
?>
...
<html lang="fr">
    <head>
        ...
        <title>YouStats - Toutes les vidéos</title>
    </head>
    <body>
        <header>
            ...
            <ul>
                ...
                <li><a href="#">Toutes les vidéos</a></li>
        ...
        <main>
            ...
            <section id="resume">
                ...
                <p><?php echo("Nombre total de vidéos : ".$compteur_videos["compteur_videos"]); // On affiche le nombre total de vidéos enregistrées dans la base de données ?></p>
                <form action="" method="GET">
                    <label for="tri_type">Trier par :</label>
                    <select name="tri_type" id="tri_type">
                        <option value="vues" <?php if (isset($_GET["tri_type"]) && $_GET["tri_type"] == "vues") { echo("selected='selected'"); } // On sélectionne automatiquement l'option choisie par l'utilisateur lors de son précédent tri (2 fois => 1 fois pour chaque "select" et uniquement si l'utilisateur a réalisé un tri) ?>>Nombre de vues</option>
                        <option value="likes" <?php if (isset($_GET["tri_type"]) && $_GET["tri_type"] == "likes") { echo("selected='selected'"); } ?>>Nombre de likes</option>
                        <option value="commentaires" <?php if (isset($_GET["tri_type"]) && $_GET["tri_type"] == "commentaires") { echo("selected='selected'"); } ?>>Nombre de commentaires</option>
                        <option value="duree" <?php if (isset($_GET["tri_type"]) && $_GET["tri_type"] == "duree") { echo("selected='selected'"); } ?>>Durée</option>
                    </select>
                    <select name="tri_ordre">
                        <option value="decroissant" <?php if (isset($_GET["tri_ordre"]) && $_GET["tri_ordre"] == "decroissant") { echo("selected='selected'"); } ?>>Décroissant (+ au -)</option>
                        <option value="croissant" <?php if (isset($_GET["tri_ordre"]) && $_GET["tri_ordre"] == "croissant") { echo("selected='selected'"); } ?>>Croissant (- au +)</option>
                    </select>
