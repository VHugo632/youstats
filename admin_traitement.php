...
if (isset($_GET["choix_action"]) == false || isset($_GET["choix_table"]) == false) {    // On se fait automatiquement rediriger vers la page de connexion si on tente d'accéder à la page actuelle sans avoir saisi l'action à effectuer ni la table concernée par cette-dernière
...
<html lang="fr">
    <head>
        ...
        <title>YouStats - Administration - 2</title>
    </head>
    <body>
        <header>
        ...
        <main id="traitement">
            <div id="lumiere"></div>
            <?php if (isset($_GET["choix_table_id"]) == false && $_GET["choix_action"] == "mettre_a_jour") {  // On appelle la fonction "ChoixTableID" pour demander à l'administrateur l'enregistrement précis à modifier de la table choisie précédemment (uniquement pour l'action "mettre à jour") ?>
            
            <form action="" method="GET">
                <?php ChoixTableID($_GET["choix_table"]); ?>
            </form>
            
            <?php } else { ?>
            
            <form action="executions_sql.php" method="GET">
                <?php if ($_GET["choix_action"] == "ajouter") {   // On appelle les fonctions "CreationFormulaire" en fonction de l'action choisie précédemment par l'administrateur (3 fois => ajouter = CreationFormulaireAjouter, mettre à jour = CreationFormulaireMettreAJour, supprimer = CreationFormulaireSupprimer) et de l'enregistrement précis à modifier (ici, uniquement pour l'action "mettre à jour")
                    CreationFormulaireAjouter($_GET["choix_table"]);
                }
                elseif ($_GET["choix_action"] == "mettre_a_jour") {
                    CreationFormulaireMettreAJour($_GET["choix_table"], $_GET["choix_table_id"]);
                }
                else {
                    CreationFormulaireSupprimer($_GET["choix_table"]);
                }
                ?>
            </form>
